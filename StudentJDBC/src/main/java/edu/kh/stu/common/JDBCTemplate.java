...
import edu.kh.stu.common.JDBCTemplate;
...
public class JDBCTemplate {
	...
	public static Connection getConnection() {
		...
		try {
			...
			if (conn != null && !conn.isClosed()) { // NOT 연산자가 붙어서 닫히지 않았다면 이다.
				return conn;
			}
			 ...
			 * driver.xml 작성하여 git이 관리 x
			 * 
			 * 이유 2: 혹시라도 DB 연결 정보가 변경될 경우 Java코드가 아닌 읽어오는 파일의 내용을 수정하면 되기 때문에 Java 코드 수정 X
			 * -> 추가 재컴파일 필요 X
			 * 
			 * 
			 */
